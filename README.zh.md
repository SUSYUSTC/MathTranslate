...
4. 编译您的 tex 文件。您可以用 texlive 的命令 `xelatex output.tex` 编译。中文翻译需要 xeCJK 包。如果是下载的 arxiv 项目我们建议把所有文件压缩成 zip 文件后上传到 overleaf 在线编译。**注意，您需要在 `Menu - Compiler` 中设置成 XeLatex 编译器，否则无法处理其他语言。**
