...
class TextTranslator:
    def __init__(self, engine, language_to, language_from, proxy_host):
        self.engine = engine
        if engine == "google":
            from mathtranslate.mtranslate import GoogleTranslate
            self.translator = GoogleTranslate(proxy_host)
        elif engine == "tencent":
    ...
    def translate(self, text):
        ...
        while True:
            try:
                ...
                if hasattr(
                    self.translator, "is_error_request_frequency"
                ) and self.translator.is_error_request_frequency(e):
                    time.sleep(0.5)
                else:
                    raise e
...
class LatexTranslator:
    def __init__(
        self, translator: TextTranslator, debug=False, threads=0, proxy_host=None
    ):
        self.translator = translator
        self.debug = debug
        if self.debug:
    ...
    def _translate_text_in_paragraph_latex(self, latex_original_paragraph):
        ...
        for format_name in format_list:
            latex_original_paragraph = process_latex.delete_specific_format(
                latex_original_paragraph, format_name
            )

        text_original_paragraph, objs = process_latex.replace_latex_objects(
            latex_original_paragraph
        )
        # Since \n is equivalent to space in latex, we change \n back to space
        # otherwise the translators view them as separate sentences
        text_original_paragraph = process_latex.combine_split_to_sentences(
            text_original_paragraph
        )
        text_original_paragraph = process_text.split_too_long_paragraphs(
            text_original_paragraph
        )
        ...
        text_translated_paragraph = self.translate_paragraph_text(
            text_original_paragraph
        )
        if self.debug:
            print(f"\n\nParagraph {self.num}\n\n", file=self.f_new)
        ...
        latex_translated_paragraph, nbad, ntotal = process_latex.recover_latex_objects(
            text_translated_paragraph, objs, tolerate_error=True
        )
        self.nbad += nbad
        self.ntotal += ntotal
    ...
    def translate_latex_all_objects(self, latex):
        ...
        for env_name in environment_list + self.theorems:
            latex = process_latex.process_specific_env(
                latex, translate_function, env_name
            )
            latex = process_latex.process_specific_env(
                latex, translate_function, env_name + r"\*"
            )
        for command_name in command_list:
            latex = process_latex.process_specific_command(
                latex, translate_function, command_name
            )
            latex = process_latex.process_specific_command(
                latex, translate_function, command_name + r"\*"
            )
        for command_group in mularg_command_list:
            latex = process_latex.process_mularg_command(
                latex, translate_function, command_group
            )
        return latex

    def translate_text_in_paragraph_latex_and_leading_brace(
        self, latex_original_paragraph
    ):
        # it acts recursively, i.e. it also translates braces inside braces
        latex_translated_paragraph = self.translate_text_in_paragraph_latex(
            latex_original_paragraph
        )
        latex_translated_paragraph = process_latex.process_leading_level_brace(
            latex_translated_paragraph,
    ...
    def translate_paragraph_latex(self, latex_original_paragraph):
        ...
        latex_translated_paragraph = self.translate_latex_all_objects(
            latex_translated_paragraph
        )
        return latex_translated_paragraph
    ...
    def split_latex_to_paragraphs(self, latex):
        ...
        paragraphs_latex = [
            process_latex.recover_latex_objects(paragraph_text, objs)[0]
    ...
    def worker(self, latex_original_paragraph):
        try:
            if self.add_cache:
                ...
                latex_translated_paragraph = cache.load_paragraph(
                    self.hash_key, hash_key_paragraph
                )
                if latex_translated_paragraph is None:
                    latex_translated_paragraph = self.translate_paragraph_latex(
                        latex_original_paragraph
                    )
                    cache.write_paragraph(
                        self.hash_key, hash_key_paragraph, latex_translated_paragraph
                    )
            else:
                latex_translated_paragraph = self.translate_paragraph_latex(
                    latex_original_paragraph
    ...
    def translate_full_latex(self, latex_original, make_complete=True, nocache=False):
        ...
        if self.add_cache:
            cache.remove_extra()
            self.hash_key = cache.deterministic_hash(
                (
                    latex_original,
                    __version__,
                    self.translator.engine,
                    self.translator.language_from,
                    self.translator.language_to,
                )
            )
        ...
        if self.complete:
            ...
            latex_original, tex_begin, tex_end = process_latex.split_latex_document(
                latex_original, r"\begin{document}", r"\end{document}"
            )
            tex_begin = process_latex.remove_blank_lines(tex_begin)
            tex_begin = process_latex.insert_macro(
                tex_begin, "\\usepackage{xeCJK}\n\\usepackage{amsmath}"
            )
        else:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            latex_translated_paragraphs = list(
                tqdm.auto.tqdm(
                    executor.map(self.worker, latex_original_paragraphs),
        ...
        self.num = "title"
        latex_translated = process_latex.process_specific_command(
            latex_translated, self.translate_text_in_paragraph_latex, "title"
        )
        ...
        print(
            self.ntotal - self.nbad,
            "/",
            self.ntotal,
            "latex object are correctly translated",
        )

        return latex_translated


def translate_single_tex_file(
    input_path, output_path, engine, l_from, l_to, debug, nocache, threads, proxy_host
):
    text_translator = TextTranslator(engine, l_to, l_from, proxy_host)
